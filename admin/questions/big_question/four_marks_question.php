...
if (isset($_POST['submit'])) {
	$stream_id = isset($_POST['stream_id']) ? trim($_POST['stream_id']) : "2";
	$departments_id = isset($_POST['departments_id']) ? trim($_POST['departments_id']) : "1";
	$degree_id = isset($_POST['degree_id']) ? trim($_POST['degree_id']) : "1";
	$class_id = isset($_POST['class_id']) ? trim($_POST['class_id']) : "1";
	$subject_id = isset($_POST['subject_id']) ? trim($_POST['subject_id']) : "4";
	$unit_id = isset($_POST['unit_id']) ? trim($_POST['unit_id']) : "5";
	$question = isset($_POST['question']) ? addslashes(trim($_POST['question'])) : "";
	$answer = isset($_POST['answer']) ? addslashes(trim($_POST['answer'])) : "";
        ...
        <div class="col-sm-12">
            ...
            <div class="div-form">
                <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                    ...
                    <!--                                <option value="-->
					<?php //echo $department['stream_id']; ?><!--">--><?php //echo $department['dept_name']; ?>
                    <!--                                </option>-->
                    <!--                            --><?php //endforeach; ?>
                    <!--                        --><?php //endif; ?>
                    <!--                    </select>-->
					...
					<?php //echo $degree['degree_name']; ?><!--</option>-->
                    <!--                            --><?php //endforeach; ?>
                    <!--                        --><?php //endif; ?>
                    <!--                    </select>-->
                    <!---->
                    <!--                    <label for="class_id" onchange="updateSubjects(this.value)">Class Name</label>-->
                    ...
                    <!--                        --><?php
					//                        if ($units = fetchData(array('table' => 'units', 'condition' => ''))):
					//                            // Extract unique unit names
					//                            $uniqueUnitNames = array_unique(array_column($units, 'unit_name'));
					//
					//                            // Loop through unique unit names
					//                            foreach ($uniqueUnitNames as $unitName):
					//                                // Find the first occurrence of the unit name to get its ID
					//                                $firstOccurrence = current(array_keys(array_column($units, 'unit_name'), $unitName));
					//                                $unitId = $units[$firstOccurrence]['id'];
					//                                ?>
                    ...
                    <input type="text" class="input-text" id="question" name="question"
                           placeholder="ex.(tell me about your self?)" required>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#departments_id').change(function () {
            var degreeId = $("#departments_id").val();
            $.ajax({
                url: 'helper.php',
                method: 'post',
                data: {
                ...
                success: function (data) {
                    $('#degree_id').html(data);
                    console.log('Selected degreeId:', degreeId);
                },
                error: function (xhr, status, error) {
                    console.error('AJAX Error:', error);
                }
