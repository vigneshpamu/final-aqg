...
if (isset($_POST['submit'])) {
	...
	$sql = "INSERT INTO Units (stream_id, departments_id, degree_id, class_id, subject_id, unit_name) VALUES ('$stream_id', '$departments_id', '$degree_id', '$class_id', '$subject_id', '$unit_name')";
	$stmt = $db->prepare($sql);
...
if (isset($_POST['edit'])) {
	...
	$subject_id = trim($_POST['subject_id']);
	$unit_name = trim($_POST['unit_name']);
	$sql = "UPDATE units SET stream_id = '$stream_id', departments_id = '$departments_id', degree_id='$degree_id', class_id='$class_id', subject_id='$subject_id', unit_name='$unit_name' WHERE id = '$id'";
	$stmt = $db->prepare($sql);
	if ($stmt->execute()) {
